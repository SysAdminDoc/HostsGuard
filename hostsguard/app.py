#!/usr/bin/env python3
"""
HostsGuard v3.17.0 - Network Privacy Manager
...
def _dependency_constraints_path():
    ...
    try:
        ...
        candidates.extend([here / CONSTRAINTS_FILENAME, here.parent / CONSTRAINTS_FILENAME])
    except Exception:
        pass
    if _is_frozen():
        try: candidates.append(Path(sys.executable).resolve().parent / CONSTRAINTS_FILENAME)
        except Exception: pass
        meipass=getattr(sys,"_MEIPASS",None)
        if meipass: candidates.append(Path(meipass) / CONSTRAINTS_FILENAME)
    candidates.append(Path(CONSTRAINTS_FILENAME))
    for p in candidates:
        try:
...
def _maybe_bootstrap():
    if os.environ.get("HOSTSGUARD_SKIP_BOOTSTRAP")=="1": return
    _bootstrap()
...
# ─── Constants ──────────────────────────────────────────────────────────────
APP="HostsGuard"; VER="3.17.0"; FW_PFX="HG_"
...
if _PORTABLE:
    _base=Path(sys.executable).resolve().parent if getattr(sys,'frozen',False) else SOURCE_ROOT
...
class Splash(QWidget):
    ...
    def setup(s):
        ...
        s._step=QLabel(T("app.initializing","Initializing...")); s._step.setAlignment(Qt.AlignCenter)
...
class LoadingOverlay(QWidget):
    ...
    def __init__(s,parent):
        ...
        s._label=QLabel(T("app.loading","Loading..."),s); s._label.setAlignment(Qt.AlignCenter)
...
class HostsActivityTab(QWidget):
    ...
    def _build(s):
        ...
        s.search=QLineEdit(); s.search.setPlaceholderText(T("hosts_activity.search_placeholder","Search: domain:ads !microsoft reason:blocklist")); s.search.setFixedHeight(_dp(30))
        ...
        tb.addWidget(_tbtn(T("hosts_activity.scan","Scan"),"primary",s._scan,55)); lo.addLayout(tb)
...
class FWActivityTab(QWidget):
    ...
    def _build(s):
        ...
        lo.addLayout(sr)
        tb=QHBoxLayout(); tb.setSpacing(_dp(5))
        s.search=QLineEdit(); s.search.setPlaceholderText(T("firewall_activity.search_placeholder","Search: proc:chrome ip!=127.0.0.1 !lan"))
        ...
        s.info=QLabel(""); s.info.setStyleSheet(f"color:{C['dim']};font-size:{_dp(10)}px;"); ib.addWidget(s.info)
        ib.addStretch()
        s.lock_cb=QCheckBox(T("firewall_activity.lockdown","Lockdown")); s.lock_cb.setChecked(load_cfg().get('lockdown',False))
        ...
        s.obs_cb=QCheckBox(T("firewall_activity.observe","Observe")); s.obs_cb.setChecked(s.learn.observe)
        ...
        s.learn_cb=QCheckBox(T("firewall_activity.learning","Learning")); s.learn_cb.setChecked(s.learn.enabled)
...
class HostsTab(QWidget):
    ...
    def _build(s):
        ...
        desc=QLabel(T("hosts.domains_description","Blocked domains are written to your hosts file as 0.0.0.0. Allowed domains are excluded from blocking."))
        ...
        s.d_search=QLineEdit(); s.d_search.setPlaceholderText(T("hosts.search_placeholder","Search: domain:ads source:list !telemetry")); s.d_search.setFixedHeight(_dp(30))
        ...
        tr.addWidget(_tbtn(T("hosts.refresh","Refresh"),"dim",s._sync_and_load,65))
        tr.addWidget(_tbtn(T("hosts.add_domain","Add Domain"),"primary",s._add,88)); tr.addWidget(_tbtn(T("hosts.sync_to_hosts","Sync to Hosts"),"dim",s._sync,105))
        dl.addLayout(tr)
        ...
        s._sub.addTab(dw,T("tabs.managed_domains","Managed Domains"))
        # Editor
        ew=QWidget(); el=QVBoxLayout(ew); el.setContentsMargins(0,_dp(6),0,0); el.setSpacing(_dp(6))
        ed=QLabel(T("hosts.raw_description","Direct editing of {path}. Save writes immediately.",path=HOSTS_PATH)); ed.setWordWrap(True)
        ed.setStyleSheet(f"color:{C['dim']};font-size:{_dp(10)}px;"); el.addWidget(ed)
        eb=QHBoxLayout(); eb.setSpacing(_dp(5))
        eb.addWidget(_tbtn("\u21BB "+T("hosts.reload","Reload"),"dim",s._reload,75)); eb.addWidget(_tbtn(T("hosts.save","Save"),"primary",s._save,55))
        eb.addWidget(_tbtn(T("hosts.clean_save","Clean & Save"),"success",s._clean,100)); eb.addWidget(_tbtn(T("hosts.backup","Backup"),"dim",s._backup,65))
        eb.addWidget(_tbtn(T("hosts.restore","Restore"),"dim",s._restore,65)); eb.addWidget(_tbtn("\u26A0 "+T("hosts.reset","Reset"),"danger",s._unlock,65))
        eb.addStretch()
        ...
        s._sub.addTab(ew,T("tabs.raw_hosts_file","Raw Hosts File"))
        # Blocklists
        bw=QWidget(); bl=QVBoxLayout(bw); bl.setContentsMargins(0,_dp(6),0,0); bl.setSpacing(_dp(6))
        bd=QLabel(T("hosts.blocklists_description","Import community blocklists. Each adds 0.0.0.0 entries for ad/tracking/malware domains."))
        ...
        btb.addWidget(_tbtn(T("hosts.import_selected","Import Selected"),"primary",s._imp_sel,115)); btb.addWidget(_tbtn(T("hosts.select_all","Select All"),"dim",s._sel_all,76))
        btb.addWidget(_tbtn(T("hosts.clear","Clear"),"dim",s._clr_all,56)); btb.addStretch()
        ...
        mg=QGroupBox(T("hosts.paste_domains","Paste Domains")); mgl=QVBoxLayout(mg)
        s.paste=QPlainTextEdit(); s.paste.setPlaceholderText(T("hosts.paste_placeholder","Paste domains, one per line")); s.paste.setAccessibleName(T("hosts.paste_domains","Paste domains")); s.paste.setMaximumHeight(_dp(60)); mgl.addWidget(s.paste)
        mr=QHBoxLayout(); mr.addWidget(_tbtn(T("hosts.add_to_hosts","Add to Hosts"),"primary",s._paste_h,100)); mr.addWidget(_tbtn(T("hosts.database_only","Database Only"),"dim",s._paste_db,105)); mr.addStretch()
        mgl.addLayout(mr); bl.addWidget(mg)
        sg=QGroupBox(T("hosts.auto_refresh","Auto-Refresh")); sgl=QVBoxLayout(sg)
        ...
        sr.addWidget(_tbtn(T("hosts.subscribe_checked","Subscribe Checked"),"dim",s._subscribe,130)); sr.addStretch()
        sgl.addLayout(sr); bl.addWidget(sg)
        ag=QGroupBox(T("hosts.allowlist_subscriptions","Allowlist Subscriptions")); agl=QVBoxLayout(ag)
        ad=QLabel(T("hosts.allowlist_description","Domains from these URLs are whitelisted (never blocked), overriding blocklists. One URL per line."))
        ...
        ar=QHBoxLayout(); ar.addWidget(_tbtn(T("hosts.save_apply_now","Save & Apply Now"),"primary",s._apply_allowlists,140))
        ...
        s._sub.addTab(bw,T("tabs.blocklists","Blocklists"))
        ...
        sd=QLabel(T("hosts.services_description","One-click block popular services via the hosts file. Best-effort (exact hostnames, no wildcards); pair with Block Encrypted DNS so apps can't bypass it."))
        ...
        s._sub.addTab(sw,T("tabs.services","Services"))
        lo.addWidget(s._sub)
...
class FirewallTab(QWidget):
    ...
    def _build(s):
        lo=QVBoxLayout(s); lo.setContentsMargins(_dp(16),_dp(12),_dp(16),_dp(8)); lo.setSpacing(_dp(8))
        desc=QLabel(T("firewall.description","Windows Firewall rules. HostsGuard rules use the HG_ prefix.")); desc.setWordWrap(True)
        desc.setStyleSheet(f"color:{C['dim']};font-size:{_dp(10)}px;"); lo.addWidget(desc)
        tb=QHBoxLayout(); tb.setSpacing(_dp(5))
        s.fw_s=QLineEdit(); s.fw_s.setPlaceholderText(T("firewall.search_placeholder","Search: action:block program:chrome source:hostsguard !orphan")); s.fw_s.setFixedHeight(_dp(30))
        ...
        tb.addWidget(_tbtn("\u21BB "+T("firewall.refresh","Refresh"),"primary",s._refresh,75))
        tb.addWidget(_tbtn("+ "+T("firewall.new_rule","Rule"),"dim",s._new,60)); lo.addLayout(tb)
        qa=QHBoxLayout(); qa.setSpacing(_dp(5))
        qa.addWidget(_tbtn(T("firewall.block_ip_out","Block IP Out"),"danger",s._qblock_ip,85))
        qa.addWidget(_tbtn(T("firewall.block_ip_both","Block IP In+Out"),"danger",s._qblock_ip_both,105))
        qa.addWidget(_tbtn(T("firewall.block_program","Block Program"),"danger",s._qblock_prog,110))
        qa.addWidget(_tbtn(T("firewall.enable_profiles","Enable Profiles"),"dim",s._profiles,110))
        qa.addWidget(_tbtn(T("firewall.save_baseline","Save Baseline"),"dim",s._save_baseline,95))
        qa.addWidget(_tbtn(T("firewall.show_drift","Show Drift"),"dim",s._show_drift,80)); qa.addStretch()
        qa.addWidget(_tbtn(T("firewall.delete_hg_rules","Delete HG Rules"),"danger",s._del_all_hg,115)); lo.addLayout(qa)
...
class ToolsTab(QWidget):
    ...
    def _build(s):
        ...
        g1=QGroupBox(T("tools.dns_network","DNS + Network")); l1=QVBoxLayout(g1); l1.setSpacing(_dp(4))
        l1.addWidget(_tbtn(T("tools.flush_dns","Flush DNS"),"primary",s._flush)); l1.addWidget(_tbtn(T("tools.winsock_reset","Winsock Reset"),"dim",s._winsock))
        l1.addWidget(_tbtn(T("tools.dhcp_renew","DHCP Renew"),"dim",s._renew))
        l1.addWidget(_tbtn(T("tools.check_browser_doh","Check Browser DoH"),"dim",s._check_browser_doh))
        s._doh_refresh_btn=_tbtn(T("tools.refresh_doh_list","Refresh DoH List"),"dim",s._refresh_doh_intel)
        l1.addWidget(s._doh_refresh_btn)
        s._doh_cb=QCheckBox(T("tools.block_encrypted_dns","Block Encrypted DNS (DoH/DoT)"))
        ...
        s._tel_cb=QCheckBox(T("tools.block_windows_telemetry","Block Windows Telemetry"))
        ...
        dr.addWidget(s._dns_cb,1); dr.addWidget(_btn(T("tools.apply","Apply"),"primary",s._apply_dns)); l1.addLayout(dr)
        l1.addWidget(_tbtn(T("tools.scheduled_blocking","Scheduled Blocking..."),"dim",s._open_schedules))
        s._rec_btn=_tbtn(T("tools.record_session","Record Session"),"dim",s._toggle_rec); l1.addWidget(s._rec_btn)
        s._recording=False; s._rec_data=[]
        l1.addStretch(); grid.addWidget(g1)
        g2=QGroupBox(T("tools.config_data","Config + Data")); l2=QVBoxLayout(g2); l2.setSpacing(_dp(4))
        l2.addWidget(_tbtn(T("tools.export_config","Export Config"),"primary",s._export)); l2.addWidget(_tbtn(T("tools.import_config","Import Config"),"dim",s._import))
        l2.addWidget(_tbtn(T("tools.export_connections","Export Connections"),"dim",s._export_conns))
        l2.addWidget(_tbtn(T("tools.export_support_bundle","Export Support Bundle"),"dim",s._support_bundle))
        scale_row=QHBoxLayout(); scale_row.setSpacing(_dp(5)); scale_row.addWidget(QLabel(T("tools.ui_scale","UI Scale")))
        ...
        l2.addWidget(_tbtn(T("tools.prune_history","Prune History (30d)"),"dim",s._prune)); l2.addWidget(_tbtn(T("tools.clear_favicons","Clear Favicons"),"dim",s._clear_fav))
        l2.addWidget(_tbtn(T("tools.open_config_folder","Open Config Folder"),"dim",s._open)); l2.addStretch(); grid.addWidget(g2)
        g3=QGroupBox(T("tools.learning_mode","Learning Mode")); l3=QVBoxLayout(g3); l3.setSpacing(_dp(4))
        s.learn_st=QLabel(""); l3.addWidget(s.learn_st)
        l3.addWidget(_tbtn(T("tools.view_trusted","View Trusted"),"dim",s._show_t)); l3.addWidget(_tbtn(T("tools.view_untrusted","View Untrusted"),"dim",s._show_u))
        l3.addWidget(_tbtn(T("tools.clear_all_trust","Clear All Trust"),"danger",s._clear_t)); l3.addStretch(); grid.addWidget(g3)
        # Recovery section
        g4=QGroupBox(T("tools.backup_recovery","Backup + Recovery")); l4=QVBoxLayout(g4); l4.setSpacing(_dp(4))
        s.rec_st=QLabel(""); s.rec_st.setWordWrap(True); s.rec_st.setStyleSheet(f"color:{C['dim']};font-size:{_dp(10)}px;"); l4.addWidget(s.rec_st)
        l4.addWidget(_tbtn(T("tools.restore_hosts_from_db","Restore Hosts from DB"),"primary",s._restore_hosts))
        l4.addWidget(_tbtn(T("tools.restore_firewall_rules","Restore Firewall Rules"),"primary",s._restore_fw))
        l4.addWidget(_tbtn(T("tools.sync_hosts_to_db","Sync Hosts File to DB"),"dim",s._sync_hosts_db))
        l4.addWidget(_tbtn(T("tools.use_machine_policy","Use Machine Policy"),"dim",s._migrate_policy))
        l4.addWidget(_tbtn(T("tools.backup_hosts_now","Backup Hosts Now"),"dim",lambda:(s.hm.backup(),s._toast("Backed up",C['green']))))
        l4.addWidget(_tbtn(T("tools.save_current_to_profile","Save Current to Profile"),"dim",s._save_profile))
        l4.addWidget(_tbtn(T("tools.rebaseline","Re-baseline (accept current)"),"dim",s._rebaseline))
        l4.addWidget(_tbtn(T("tools.harden_hosts_acl","Harden Hosts ACL"),"dim",s._harden_acl))
        l4.addWidget(_tbtn(T("tools.restore_from_stevenblack","Restore from StevenBlack"),"dim",s._restore_upstream))
        s._ar_cb=QCheckBox(T("tools.auto_restore_on_tamper","Auto-restore on tamper")); s._ar_cb.setChecked(load_cfg().get('auto_restore_on_tamper',False))
        ...
        lo.addLayout(grid)
        lg=QGroupBox(T("tools.event_log","Event Log")); ll=QVBoxLayout(lg)
        lr=QHBoxLayout(); lr.setSpacing(_dp(5))
        s.log_s=QLineEdit(); s.log_s.setPlaceholderText(T("tools.event_log_search_placeholder","Search: action:blocked reason:firewall !telemetry")); s.log_s.setFixedHeight(_dp(28))
        ...
        lr.addWidget(_tbtn(T("tools.clear_log","Clear Log"),"danger",s._clear_log,90,"Delete all event log rows")); ll.addLayout(lr)
...
class MainWindow(QMainWindow):
    def __init__(s,results):
        super().__init__(); s.setWindowTitle(T("app.window_title","{app} v{version}",app=APP,version=VER))
    ...
    def _build_ui(s):
        ...
        s._hosts_act=HostsActivityTab(s.db,s.hm,s.learn); s._tabs.addTab(s._hosts_act,T("tabs.hosts_activity","Hosts Activity"))
        s._fw_act=FWActivityTab(s.db,s.hm,s.cdb,s.learn); s._tabs.addTab(s._fw_act,T("tabs.firewall_activity","Firewall Activity"))
        s._hosts_tab=HostsTab(s.db,s.hm); s._tabs.addTab(s._hosts_tab,T("tabs.hosts_file","Hosts File"))
        s._fw_tab=FirewallTab(); s._tabs.addTab(s._fw_tab,T("tabs.firewall_rules","Firewall Rules"))
        s._tools=ToolsTab(s.db,s.hm,s.cdb,s.learn); s._tabs.addTab(s._tools,T("tabs.tools","Tools"))
